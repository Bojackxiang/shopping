...
import AddressForm, { useAddressForm } from "@/components/form/address-form";

export default function AddressesContent() {
  const { data, error, isLoading } = useAddress();
  const { open, currentAddress, openDialog, setOpen } = useAddressForm();
  ...
  const handleEditAddress = (address: NonNullable<typeof data>[0]) =>
    openDialog(address);

  const handleAddNewAddress = () => openDialog();
